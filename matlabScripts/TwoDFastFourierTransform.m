...
directoriesList = dir( pathToFileList );
fileList = ls( pathToFileList );

%%% ________________________________________________________________________________________________________________________________ %%%

% Extract the matrix
for sampleDir = 3 : numel(directoriesList)
    % Define variables for each sample file %
    pathToFile = fullfile( directoriesList( sampleDir ).folder, directoriesList( sampleDir ).name);

    sampleMatrix = table2array( readtable( pathToFile, opts ) );
    ...
    % Actually it shows the last image
    result2dFourierTransform = imagesc( abs( log2( fftshift( fast2DFourierTransform ) ) ) );
