...
// 	std::vector<std::uint8_t>::const_iterator e = root_iter.end();
	...
	*/
	...
	int max_n = 2;
	unsigned zero_count = 5; // The 1-indexed zeros to compute
	// Initialize vector of size max_n by zero_count
	std::vector<std::vector<int>>jn_zeros(max_n,std::vector<int>(zero_count));

	try {
		for (int n = 0; n < max_n; n++) {
			for (unsigned z = 0; z < zero_count; z++) {
				double root = boost::math::cyl_bessel_j_zero(n*1.0, z+1);
				// Displaying with default precision of 6 decimal digits:
				std::cout << n << ", " << z << ": " << root << std::endl; // 2.40483
				jn_zeros[n][z] = root;
			}
		}
		/*
