...
class PredictorRF(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, df: pd.DataFrame):
        ...
        dfc["q_cluster_label"] = dfc["type_measurement"].apply(
            lambda x: 0 if x == "SAXS" else 1
        )

        dfs = dfc[dfc["type_measurement"] == "SAXS"].copy()
        dfw = dfc[dfc["type_measurement"] == "WAXS"].copy()

        self.ML_saxs = MLCluster(
            df=dfs, q_cluster=0, q_range=dfs.q_range.iloc[0]
        )
        self.ML_waxs = MLCluster(
            df=dfw, q_cluster=1, q_range=dfw.q_range.iloc[0]
        )
        ...
        if self.show_result:
            print(
                f"""Accuracy: {mls.accuracy}, ROC: {mls.roc_auc},
                q_max: {mls.df.q_range_max.mean()},
                q_label {mls.df.q_cluster_label.mean()}"""
            )
            print(
                f"""Accuracy: {mlw.accuracy}, ROC: {mlw.roc_auc},
                q_max: {mlw.df.q_range_max.mean()},
    ...
    def predict(self, df):
        ...
        combined_pred_df = pd.concat([saxs_pred_df, waxs_pred_df]).loc[
            dfc.index
        ]
    ...
    def predict_proba(self, df):
        ...
        saxs_pred = self.ML_saxs.model.predict_proba(
            PredictorRF.prep_data(dfs)
        )
        waxs_pred = self.ML_waxs.model.predict_proba(
            PredictorRF.prep_data(dfw)
        )
