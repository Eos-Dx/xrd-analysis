...
from xrdanalysis.data.functions import get_center
...
class AzimuthalIntegration(TransformerMixin):
    ...
    def transform(self, x: pd.DataFrame) -> pd.DataFrame:
        ...
        integration_results = x_copy.apply(
            lambda row: perform_azimuthal_integration(row, self.npt, mask), axis=1
        )
...
class DataPreparation(TransformerMixin):
    ...
    def transform(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        'study_name', 'study_id', 'cancer_tissue', 'cancer_diagnosis', 'patient_id', 'wavelength',
        'pixel_size', 'calibration_manual_distance', 'center_col', 'center_row',
        'calculated_distance', 'measurement_data', 'center'
        """
        dfc = df.copy()
        dfc = dfc[~dfc['center_col'].isna() | ~dfc['center_row'].isna() | ~dfc['calculated_distance'].isna()]

        def is_all_none(array):
            return all(x is None for x in array)

        # Apply this function to the 'measurement_data' column and filter the DataFrame
        dfc = dfc[~dfc['measurement_data'].apply(is_all_none)]

        dfc['measurement_data'] = dfc['measurement_data'].apply(lambda x: np.nan_to_num(x))
        ...
        # Apply the function and filter out the rows where 'center' is (np.NaN, np.NaN)
        dfc = dfc[~dfc['center'].apply(is_nan_pair)]

        columns = ['study_name', 'study_id', 'cancer_tissue', 'cancer_diagnosis', 'patient_id', 'wavelength',
                   'pixel_size', 'calibration_manual_distance', 'center_col', 'center_row',
                   'calculated_distance', 'measurement_data', 'center']
...
class Clusterization(TransformerMixin):
    ...
    def transform(self, df: pd.DataFrame) -> pd.DataFrame:
        def remove_outliers_by_cluster(dataframe, z_score_threshold,
                                       direction='negative',
                                       num_clusters=4):
            ...
            for cluster in range(num_clusters):
                cluster_indices = dataframe[dataframe['q_cluster_label'] == cluster].index

                # Extract q_range_max values for the current cluster
                q_range_max_values = dataframe.loc[cluster_indices,
                                                   'q_range_max']
                ...
                if direction == 'both':
                    ...
                    raise ValueError("Invalid direction. Use 'both', 'positive', \
                                      or 'negative'.")
