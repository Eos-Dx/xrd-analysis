...
def calculate_pixel_to_mm_ratio(pixel_distance: float, mm_distance: float) -> float:
    ...
    logger.info("Calculated pixel to mm ratio", ratio=ratio, 
                pixel_dist=pixel_distance, mm_dist=mm_distance)
    return ratio


def validate_stage_position(x_mm: float, y_mm: float, 
                          limits: Tuple[float, float, float, float]) -> bool:
    ...
    if not valid:
        logger.warning("Stage position outside limits", 
                      x=x_mm, y=y_mm, limits=limits)
