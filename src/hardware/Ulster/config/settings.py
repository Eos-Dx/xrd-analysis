...
class ConfigManager:
    ...
    def __init__(self, config_path: Optional[Path] = None):
        if config_path is None:
            config_path = Path(__file__).parent.parent / "resources" / "config" / "main.json"
    ...
    def load_config(self):
        ...
        try:
            ...
            logger.error("Error loading configuration", error=str(e), path=str(self.config_path))
            self._create_default_config()
    ...
    def save_config(self):
        ...
        try:
            ...
            config_data = {
                ...
                "default_folder": self._app_settings.default_folder if self._app_settings else "",
                "default_image_folder": self._app_settings.default_image_folder if self._app_settings else "",
                "default_image": self._app_settings.default_image if self._app_settings else "",
                "log_level": self._app_settings.log_level if self._app_settings else "INFO",
                "log_to_file": self._app_settings.log_to_file if self._app_settings else True,
            ...
            logger.error("Error saving configuration", error=str(e), path=str(self.config_path))
    ...
    def _parse_config(self):
        ...
        if stage_data:
            self._stage_config = StageConfig(
                ...
                x_limits=tuple(stage_data["x_limits"]) if "x_limits" in stage_data else None,
                y_limits=tuple(stage_data["y_limits"]) if "y_limits" in stage_data else None
            )
