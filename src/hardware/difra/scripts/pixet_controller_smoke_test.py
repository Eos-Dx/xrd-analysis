...
from hardware.difra.hardware.detectors import PixetDetectorController
...
def main() -> int:
    ...
    detector_config: dict[str, str] = {"pixet_sdk_path": args.sdk_path}
    if args.device_id.strip():
        detector_config["id"] = args.device_id.strip()

    controller = PixetDetectorController(
        alias=args.alias,
    ...
    try:
        print(f"[INFO] Initializing detector via DiFRA class: {args.alias}")
