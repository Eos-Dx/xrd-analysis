...
def create_session_container(
    ...
    filename = schema_v1.format_session_container_filename(container_id, sample_id)
    file_path = str(folder / filename)

    root_attrs = {
        schema_v1.ATTR_SAMPLE_ID: sample_id,
        schema_v1.ATTR_SESSION_ID: container_id,
        schema_v1.ATTR_CREATION_TIMESTAMP: time.strftime("%Y-%m-%d %H:%M:%S"),
        schema_v1.ATTR_ACQUISITION_DATE: acquisition_date,
        schema_v1.ATTR_OPERATOR_ID: operator_id,
        schema_v1.ATTR_SITE_ID: site_id,
        schema_v1.ATTR_MACHINE_NAME: machine_name,
        schema_v1.ATTR_BEAM_ENERGY_KEV: beam_energy_keV,
    }

    if patient_id is not None:
        root_attrs[schema_v1.ATTR_PATIENT_ID] = patient_id

    io.create_empty_container(
        file_path=file_path,
        container_id=container_id,
        container_type=schema_v1.CONTAINER_TYPE_SESSION,
        root_attrs=root_attrs,
    )

    # Create top-level groups
    io.create_group_if_missing(file_path, schema_v1.GROUP_IMAGES)
    io.create_group_if_missing(file_path, schema_v1.GROUP_IMAGES_ZONES)
    io.create_group_if_missing(file_path, schema_v1.GROUP_IMAGES_MAPPING)
    io.create_group_if_missing(file_path, schema_v1.GROUP_POINTS)
    io.create_group_if_missing(file_path, schema_v1.GROUP_MEASUREMENTS)
    io.create_group_if_missing(file_path, schema_v1.GROUP_ANALYTICAL_MEASUREMENTS)
...
def copy_technical_to_session(
    ...
    io.copy_group(
        ...
        src_group=schema_v1.GROUP_TECHNICAL,
        dst_file=session_file,
        dst_group=schema_v1.GROUP_TECHNICAL,
    )
...
def add_image(
    ...
    image_id = schema_v1.format_image_id(image_index)
    image_path = f"{schema_v1.GROUP_IMAGES}/{image_id}"

    # Create image group
    io.create_group_if_missing(file_path, image_path)
    ...
    # Set attributes
    attrs = {
        schema_v1.ATTR_IMAGE_TYPE: image_type,
        schema_v1.ATTR_TIMESTAMP: timestamp,
    }
    io.set_attrs(file_path, image_path, attrs)
...
def add_zone(
    ...
    Returns:
        Zone group path
    """
    if not schema_v1.validate_zone_role(zone_role):
        raise ValueError(f"Invalid zone role: {zone_role}")

    zone_id = schema_v1.format_zone_id(zone_index)
    zone_path = f"{schema_v1.GROUP_IMAGES_ZONES}/{zone_id}"

    # Create zone group
    io.create_group_if_missing(file_path, zone_path)
    ...
    # Set attributes
    attrs = {
        schema_v1.ATTR_ZONE_ROLE: zone_role,
        schema_v1.ATTR_ZONE_SHAPE: shape,
    }
    if holder_diameter_mm is not None:
        attrs[schema_v1.ATTR_HOLDER_DIAMETER_MM] = holder_diameter_mm

    io.set_attrs(file_path, zone_path, attrs)
...
def add_image_mapping(
    ...
    mapping_path = f"{schema_v1.GROUP_IMAGES_MAPPING}/mapping"
...
def add_point(
    ...
    Returns:
        Point group path
    """
    point_id = schema_v1.format_point_id(point_index)
    point_path = f"{schema_v1.GROUP_POINTS}/{point_id}"

    # Create point group
    io.create_group_if_missing(file_path, point_path)

    # Set attributes
    attrs = {
        schema_v1.ATTR_PIXEL_COORDINATES: np.array(pixel_coordinates),
        schema_v1.ATTR_PHYSICAL_COORDINATES_MM: np.array(physical_coordinates_mm),
        schema_v1.ATTR_POINT_STATUS: point_status,
    }
    io.set_attrs(file_path, point_path, attrs)
...
def update_point_status(
    ...
    io.set_attrs(
        file_path=file_path,
        path=point_path,
        attrs={schema_v1.ATTR_POINT_STATUS: point_status},
    )
...
def add_measurement(
    ...
    point_id = schema_v1.format_point_id(point_index)
    meas_id = schema_v1.format_measurement_id(meas_counter)
    meas_path = f"{schema_v1.GROUP_MEASUREMENTS}/{point_id}/{meas_id}"

    # Create measurement group
    io.create_group_if_missing(file_path, meas_path)

    # Set measurement-level attributes
    attrs = {
        schema_v1.ATTR_MEASUREMENT_COUNTER: meas_counter,
        schema_v1.ATTR_TIMESTAMP_START: timestamp_start,
        schema_v1.ATTR_MEASUREMENT_STATUS: measurement_status,
    }
    if timestamp_end is not None:
        attrs[schema_v1.ATTR_TIMESTAMP_END] = timestamp_end
    ...
    try:
        io.set_reference_attr(
            file_path=file_path,
            obj_path=meas_path,
            attr_name=schema_v1.ATTR_POINT_REF,
    ...
    io.set_attrs(file_path, meas_path, attrs)
    ...
    for detector_id, raw_signal in measurement_data.items():
        ...
        det_attrs = {
            schema_v1.ATTR_DETECTOR_ID: detector_id,
        }

        if detector_id in detector_metadata:
            meta = detector_metadata[detector_id]
            det_attrs[schema_v1.ATTR_INTEGRATION_TIME_MS] = meta.get(
                "integration_time_ms", 0
            )
            if "beam_energy_keV" in meta:
                det_attrs[schema_v1.ATTR_BEAM_ENERGY_KEV] = meta["beam_energy_keV"]

        io.set_attrs(file_path, detector_path, det_attrs)
        ...
        try:
            ...
            io.set_reference_attr(
                ...
                attr_name=schema_v1.ATTR_PONY_REF,
...
def add_analytical_measurement(
    ...
    ana_id = schema_v1.format_analytical_measurement_id(meas_counter)
    ana_path = f"{schema_v1.GROUP_ANALYTICAL_MEASUREMENTS}/{ana_id}"

    # Create analytical measurement group
    io.create_group_if_missing(file_path, ana_path)
    ...
    attrs = {
        ...
        schema_v1.ATTR_TIMESTAMP_START: timestamp_start,
        schema_v1.ATTR_MEASUREMENT_STATUS: measurement_status,
        schema_v1.ATTR_ANALYSIS_TYPE: analysis_type,
    }
    if timestamp_end is not None:
        attrs[schema_v1.ATTR_TIMESTAMP_END] = timestamp_end

    io.set_attrs(file_path, ana_path, attrs)
...
def link_analytical_measurement_to_point(
    ...
    ana_id = schema_v1.format_analytical_measurement_id(analytical_measurement_index)
    ana_path = f"{schema_v1.GROUP_ANALYTICAL_MEASUREMENTS}/{ana_id}"

    io.append_reference_to_list_attr(
        file_path=file_path,
        obj_path=point_path,
        attr_name=schema_v1.ATTR_ANALYTICAL_MEASUREMENT_REFS,
        ...
        for candidate in candidates:
            try:
                with io.open_h5_append(candidate) as f:
                    if f.attrs.get(schema_v1.ATTR_SAMPLE_ID) == sample_id:
