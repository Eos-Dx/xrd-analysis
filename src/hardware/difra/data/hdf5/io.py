...
def create_empty_container(
    ...
    if container_type not in [schema_v1.CONTAINER_TYPE_TECHNICAL, schema_v1.CONTAINER_TYPE_SESSION]:
        raise ValueError(f"Invalid container type: {container_type}")
    
    with h5py.File(file_path, "w") as f:
        # Write required root attributes
        f.attrs[schema_v1.ATTR_CONTAINER_ID] = container_id
        f.attrs[schema_v1.ATTR_CONTAINER_TYPE] = container_type
        f.attrs[schema_v1.ATTR_SCHEMA_VERSION] = schema_v1.SCHEMA_VERSION
    ...
    with open_h5_append(file_path) as f:
        ...
        if attr_name in obj.attrs:
            ...
            if existing_refs.size == 0:
                ...
                # Append to existing
                obj.attrs[attr_name] = np.append(existing_refs, new_ref)
        else:
